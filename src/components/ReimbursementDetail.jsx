      ...
      <ScrollArea className="flex-1 px-4 mt-4 w-full [&>[data-slot=scroll-area-viewport]]:overflow-x-hidden">
        <div className="space-y-6 pb-6">
          ...
          {(() => {
            ...
            const sortedLogs = detailData.logs
              ? [...detailData.logs].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
              : [];
            ...
            if (latestAction === 'Drafted' || latestAction === 'ManagerRevision') {
              return (
                <div className="flex justify-end pt-2">
                  <Button
                    onClick={() =>
                      navigate({ to: '/reimbursement/update', search: { id: detailData.id } })
                    }
          ...
          <div>
            <h4 className="text-sm font-semibold mb-3 flex items-center">
              <FileText className="w-4 h-4 mr-2" /> Expense Items
            </h4>
                ...
                <div key={item.id} className="border rounded-lg p-3 text-sm bg-muted/20">
                  <div className="flex justify-between mb-1">
                    <span className="font-medium text-gray-900">
                      {format(new Date(item.dateOfExpense), 'dd MMM yyyy')}
                    </span>
