...
export default function TripCalendarWidget({ tripsData, tripCalendarLoading }) {
  const { user } = useAuth();
  // const [loading, setLoading] = useState(false);
  // const [tripData, setTripData] = useState([]);
  const [selectedDate, setSelectedDate] = useState(new Date());

  const tripData = tripsData;
  const loading = tripCalendarLoading;
          ...
          <div className="space-y-3">
            {activeTrips.length > 0 ? (
              activeTrips.map((trip, idx) => (
                <div
                  key={idx}
                  className="bg-white dark:bg-slate-800 p-3 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm"
                >
