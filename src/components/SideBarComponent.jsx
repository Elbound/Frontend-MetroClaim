    ...
    <>
      ...
      <div className={wrapperClass}>
        ...
        <div className="flex-1 overflow-y-auto w-full p-4 flex flex-col h-full scrollbar-thin scrollbar-thumb-blue-400 scrollbar-track-transparent">
          {/* Header / Logo */}
          <div
            className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'} mb-6 px-2 shrink-0 ${mobileMode ? 'mt-2' : ''}`}
          >
            <img
              src={MetroLogo}
              alt="Metrodata"
              className="w-10 h-10 min-w-10 min-h-10 rounded-full shrink-0 object-cover"
            />
            {!collapsed && (
              <span className="text-lg font-bold tracking-wide whitespace-nowrap overflow-hidden transition-all duration-300">
                MetroClaim
              </span>
            )}
          </div>
          ...
          <div
            className={`flex items-center gap-3 p-3 mb-6 rounded-lg bg-white/5 border border-white/10 backdrop-blur-sm shrink-0 ${collapsed ? 'justify-center' : ''}`}
          >
            ...
            {!collapsed && (
              <div className="flex flex-col overflow-hidden transition-all duration-300">
                <span className="font-semibold truncate text-sm" title={user?.name}>
          ...
          <div className="flex flex-col flex-1">
            ...
            {isManager && (
              <>
                {!collapsed && (
                  <div className="mt-4 mb-2 px-2 text-xs font-semibold text-blue-300 uppercase tracking-wider opacity-80 whitespace-nowrap">
                    Management
                  </div>
                )}
                ...
                <Link
                  to="/reimbursement/manager-history"
                  className={linkClass}
                  activeProps={{ className: activeClass }}
                  onClick={handleLinkClick}
                >
            ...
            {isFinance && (
              <>
                ...
                <Link
                  to="/reimbursement/finance-history"
                  className={linkClass}
                  activeProps={{ className: activeClass }}
                  onClick={handleLinkClick}
                >
            ...
            <div className="mt-auto pt-4 border-t border-white/10">
              <button
                ...
                className={`${linkClass} w-full text-left text-red-300 hover:bg-red-500/20`}
              >
